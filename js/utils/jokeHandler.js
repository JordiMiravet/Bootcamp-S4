...
export const createStarRating = () => {
    let ratingChecked = null;
    // Deseleccionar checkbox
    const listenToStars = () => {
        document.querySelectorAll('input[name="rating"]').forEach(star => star.addEventListener("click", () => {
            ratingChecked === star
...
export const showJoke = async () => {
    ...
    if (!result)
        return console.error("The element 'result' was not found");
    try {
